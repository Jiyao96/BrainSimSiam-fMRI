...
def node_mask(node, soft_mask):
    # print(node.shape)
    # print(soft_mask.shape)
    node_seg = torch.einsum('ij,i->ij', node, soft_mask)
    return node_seg

def img_mask(img, soft_mask, roi_mask=None):
    roi_mask = torch.einsum('ijkl,i->ijkl', roi_mask, soft_mask)
    img_mask = roi_mask.sum(dim=0)
    img_seg = img * img_mask 
    return img_seg

_shen268_mask = None

def get_shen268_mask(device):
    global _shen268_mask
    if _shen268_mask is None or _shen268_mask.device != device:
        _shen268_mask = torch.load('datasets/shen268_atlas_mask').to(device)
    return _shen268_mask
...
class GNNExplainer(torch.nn.Module):
    ...
    def __init__(self, model, epochs: int = 100, lr: float = 0.01,
        ...
        assert edge_mask_type in ['individual']
        self.model = model
        self.epochs = epochs
        self.lr = lr
    ...
    def __loss__(self, node_idx, log_logits, pred_label):
        ...
        if self.return_type == 'regression':
            if node_idx != -1:
                ...
                loss = F.mse_loss(log_logits, pred_label)
        else:
            if node_idx != -1:
    ...
    def explain_graph(self, data, **kwargs):
        ...
        input_batch_size=x.shape[0]//temp_node_num

        # print(edge_attr.shape)

        # Get the initial prediction.
        with torch.no_grad():
            out, _, _ = self.model(data, data)
            if self.return_type == 'regression':
                prediction = out
            else:
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            # apply node masks
            node_temp_mask = self.node_feat_mask.sigmoid().squeeze().repeat(input_batch_size)
            h = node_mask(x, node_temp_mask)
            # apply img masks
            h_img = img_mask(
                img,
                self.node_feat_mask.sigmoid().squeeze(),
                roi_mask=get_shen268_mask(img.device),
            )

            # apply edge masks
            edge_temp_mask = self.edge_mask.sigmoid().squeeze()
            ...
            out, _, _ = self.model(mask_data, mask_data)
