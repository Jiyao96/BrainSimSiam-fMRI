...
from models.encoder_models import SimSiam, edge_dropout, node_feature_mask, node_img_mask
import argparse
...
parser.add_argument('--node_prob', type=float, default=0.5, help="node masking probability")
parser.add_argument('--edge_prob', type=float, default=0.5, help="edge dropping probability")
parser.add_argument('--drop_prob', type=float, default=0.1, help="roi dropping probability")
parser.add_argument('--alpha', type=float, default=1.0, help="scalar weight for loss terms")
parser.add_argument('--num_roi', type=int, default=268, help="number of brain ROI")
...
# atlas mask
shen268_mask = torch.load('datasets/shen268_atlas_mask').to(device)
print(shen268_mask.shape)

def aug(data, edge_prob=args.edge_prob, node_prob=args.node_prob, drop_prob=args.drop_prob, num_roi=args.num_roi):
    ...
    # additional joint node and image mask
    data.x, data.img = node_img_mask(data, drop_prob, num_roi, default_mask=shen268_mask)
    return data
...
def train():
    ...
    for base_data, task_aug_data in train_loader:
        ...
        task_aug_data = task_aug_data.clone()
        # send data to GPU 
...
torch.save(subject_model.state_dict(), f"weights")
